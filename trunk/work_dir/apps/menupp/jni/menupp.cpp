...
Java_srdes_menupp_menuppRenderer_renderFrame(JNIEnv *, jobject)
{
    ...
    for(int tIdx = 0; tIdx < state.getNumActiveTrackables(); tIdx++)
    {
        ...
        int textureIndex = (!strcmp(trackable->getName(), "item_1")) ? 0 : 1;
...
Java_srdes_menupp_menupp_toggleFlash(JNIEnv*, jobject, jboolean flash)
{
    return QCAR::CameraDevice::getInstance().setFlashTorchMode((flash==JNI_TRUE)) ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_srdes_menupp_menupp_autofocus(JNIEnv*, jobject)
{
    return QCAR::CameraDevice::getInstance().startAutoFocus()?JNI_TRUE:JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_srdes_menupp_menupp_setFocusMode(JNIEnv*, jobject, jint mode)
{
